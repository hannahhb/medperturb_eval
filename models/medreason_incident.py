...
def _serialize_paths(path_data: List, max_paths: int = 5) -> str:
    """
    Incident encoding — empirically best for neighbour/path tasks (Table 1,
    arxiv 2503.10970): each node lists its neighbours with edge labels.

    Example output:
        fosaprepitant: aprepitant (synergistic interaction)
        aprepitant: fosaprepitant (synergistic interaction), nausea (side effect)
        nausea: aprepitant (side effect)

    Capped at max_paths paths to avoid dense-graph degradation.
    """
    # Collect edges across all paths (deduplicated)
    # adjacency: node -> {neighbour: relation}
    ...
    for item in path_data[:max_paths]:
        if isinstance(item, tuple) and len(item) == 2:
            ...
            nodes, rels = item, [None] * (len(item) - 1)
        else:
            continue
        ...
        for i in range(len(nodes) - 1):
            ...
            if rel and rel == "parent-child":
                rel = None          # skip uninformative hierarchy edges
            label = str(rel) if rel else "related"
            adj.setdefault(u, {})[v] = label
            adj.setdefault(v, {})[u] = label   # undirected
    ...
    for node in adj:
        neighbours = ", ".join(
            f"{nbr} ({lbl})" for nbr, lbl in adj[node].items()
        )
...
class MedReasonModel(BaseAdvancedModel):
    ...
    def _map_to_kg_node(
        ...
        """
        Map an extracted entity to a PrimeKG node using type-restricted cosine
        similarity — identical logic to MedPathAgent's get_topk_similar_entities.

        Searches only within nodes of the matching entity type (not the global
        FAISS index), and discards candidates whose cosine similarity is below
        filter_threshold.
        """
        ...
        # Pre-normalised, already on device (cached in _load_sapbert)
        ...
        emb_norm = F.normalize(out, p=2, dim=-1)                      # (1, d)

        sims = (emb_norm @ node_embs_norm.T).squeeze(0).cpu()         # (N,)
        vals, idxs = torch.topk(sims, min(k, sims.size(0)))

        top1_sim = float(vals[0].item()) if vals.numel() > 0 else 0.0

        # Perfect name match (case-insensitive) — always accept
